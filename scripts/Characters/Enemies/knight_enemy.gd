...
@export var weapon_variant: WeaponVariant = WeaponVariant.SWORD
...
func set_day_state(is_corporeal: bool) -> void:
	is_world_enabled = (
		active_in_corporeal_world
		if is_corporeal
		else active_in_spectral_world
	)
...
func _begin_attack_animation() -> void:
	...
	attack_visual.play(_get_attack_animation())
...
func _on_attack_frame_changed() -> void:
	...
	var active_frame := 0 if weapon_variant == WeaponVariant.SPEAR else 2
	_set_attack_enabled(attack_visual.frame >= active_frame)
...
func _get_attack_center() -> float:
	return spear_attack_center if weapon_variant == WeaponVariant.SPEAR else sword_attack_center
...
func _can_attack_target() -> bool:
	return (
		...
		and abs(tracked_target.global_position.y - global_position.y)
